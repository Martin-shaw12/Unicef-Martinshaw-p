...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("country", "Select Country:", choices = unique(unicef_indicator_2$country), multiple = TRUE)
    ),
    mainPanel(
      plotOutput("barplot")
    )
...
server <- function(input, output) {
  output$barplot <- renderPlot({
    if (length(input$country) > 0) {  # Check if at least one country is selected
      filtered_data <- unicef_indicator_2[unicef_indicator_2$country %in% input$country, ]
    } else {
      filtered_data <- unicef_indicator_2  # If no country is selected, show all data
    }
